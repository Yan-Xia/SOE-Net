...
# wandb.init(project="place recognition", config=tf.flags.FLAGS, sync_tensorboard=True)
...
parser.add_argument('--margin', type=float, default=0.5, help='Margin for hinge loss [default: 0.2]')
...
def train():
    ...
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            # Restore a model
            #saver.restore(sess, os.path.join('log/', "model15_1.ckpt"))
            ...
            #init_step = sess.run(batch)
            #begin_epoch  = init_step // 21711 + 1
...
def train_one_epoch(sess, ops, train_writer, test_writer, epoch, saver):
    ...
    for i in range(len(train_file_idxs)//BATCH_NUM_QUERIES):
        ...
        #print('negtives shape', negatives.shape)
        ...
        if(i%1500==1):
            modelname = 'model'+str(epoch)+'_'+str(i)+'.ckpt'
            save_path = saver.save(sess, os.path.join(LOG_DIR, modelname))
            # saver.save(sess, os.path.join(wandb.run.dir, modelname))
