...
parser.add_argument('--negatives_per_query', type=int, default=8, help='Number of definite negatives in each training tuple [default: 20]')
...
def evaluate():
    ...
    with tf.Graph().as_default():
        ...
        for j in range(len(QUERY_SETS)):
            #print('QUERY_SETS[j]', QUERY_SETS[j])
...
def get_latent_vectors(sess, ops, dict_to_process):
    ...
    for q_index in range(len(train_file_idxs)//batch_num):
        ...
        #queries=rotate_point_cloud(queries)
        #queries=noising_point_cloud(queries)
        ...
        # if(q_index==2):
        #     #print(file_names)
        #     #weights = np.reshape(weights,[-1,NUM_POINTS,1])
